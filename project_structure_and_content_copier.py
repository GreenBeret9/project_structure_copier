...
def should_ignore(path, ignore_patterns):
    if '.git' in path.split(os.sep) or os.path.basename(path) == '.gitignore':
        return True
    ...
    for root, dirs, files in os.walk(startpath, topdown=True):
        ...
        for f in files:
            ...
            if not should_ignore(file_path, ignore_patterns):
                ...
                file_contents = read_file_contents(os.path.join(root, f))
                content_indent = '|   ' * (level + 1)
                for line in file_contents.splitlines():
                    print(f'{content_indent}{line}', file=output)
...
if __name__ == "__main__":
    ...
    print("\nDirectory structure with file contents has been copied to clipboard.")
